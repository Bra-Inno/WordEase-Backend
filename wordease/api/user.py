...
from app.schemas.general_response import ResponseModel
from app.schemas.user import UserRegister,UserPasswordLogin,UserUpdate,UserCodeLogin
from app.utils.user.json_web_token import create_jwt
...
@api_user.get("/register_send_verification_code", description="发送注册账号时候需要的验证码")
async def register_send_verification_code(phone : str):
    code = await get_code(phone, REDIS_USER_REGISTER_CODE)
